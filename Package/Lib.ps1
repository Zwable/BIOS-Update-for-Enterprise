    ...
    <Grid x:Name="grd_mainwindow">
        ...
        <Grid Grid.Row ="0" Background="#106FAA">
            <StackPanel Orientation="Horizontal" Grid.ColumnSpan="3">
                ...
                <TextBlock Text="BIOS/Firmware upgrade" FontWeight="Bold" FontSize="25" HorizontalAlignment="Left" VerticalAlignment="Center"/>
            </StackPanel>
        ...
        <Grid Grid.Row ="3" x:Name="grd_batteryinfo" Visibility="Visible" Background="LightGray">
            <materialDesign:Card VerticalAlignment="Stretch" HorizontalAlignment="Stretch" Margin="10 10 10 10" materialDesign:ShadowAssist.ShadowDepth="Depth2">
                <StackPanel Orientation="Horizontal">
                    ...
                    <TextBlock x:Name="tbx_percentagetext" Text="loading..." FontSize="17" VerticalAlignment="Center" HorizontalAlignment="Center" FontWeight="Bold" Margin="10 10 10 10"/>
                </StackPanel>
            </materialDesign:Card>
        </Grid>
        ...
        <materialDesign:DialogHost x:Name="pop_installfirmware" IsOpen="False" Grid.RowSpan="5">
            <materialDesign:DialogHost.DialogContent>
                <StackPanel Margin="29" Orientation="Vertical" Width="120" Height="160">
                    <TextBlock Text="Do not turn off your PC. Updating BIOS/firmware.." FontSize="15" FontWeight="Bold" TextWrapping="Wrap" TextAlignment="Center" Margin="0 0 0 0"/>
    ...
    $CheckBatteryTimer.add_Tick({
        ...
        if($ACConnected){
            $WPFWindow.Window.Dispatcher.invoke([action]{
                ...
                $WPFWindow.tbx_percentagetext.Text = if($CurrentCharge -lt 100){"Charging..."}else{"Fully charged.."}
                $WPFWindow.ico_status.Kind ="BatteryCharging$BatteryIndicator"
            },"Normal")
        }else{
            $WPFWindow.Window.Dispatcher.invoke([action]{
                $WPFWindow.tbx_percentage.Text = "$CurrentCharge`%"
                $WPFWindow.tbx_percentagetext.Text = "Not charging..."
        ...
        if((([int]$CurrentCharge -gt [int]$MinimumPercentage) -and $ACConnected) -or !(Get-IsLaptop)){
            ...
            if(!( Get-Job -State Running | Where-Object {$_.Name -eq "UpgradeBIOS"}) -and !( Get-Job -State Completed | Where-Object {$_.Name -eq "UpgradeBIOS"})){
                ...
                $ScriptBlock={
                    ...
                    if($($UpgradeInfo.Manufacturer) -eq "Dell"){
                        ...
                        #Run the upgrade
                        $Process = Start-Process -FilePath $($UpgradeInfo.TempBiosFilePath) -ArgumentList "/s /f" -WindowStyle Hidden -PassThru -Wait
                        ...
                        $Process = Start-Process -FilePath $($UpgradeInfo.TempBiosFilePath) -ArgumentList "/VERYSILENT /DIR=`"$ExtractLocation`" /Extract=`"YES`"" -WindowStyle Hidden -PassThru -Wait
                        If($Process.ExitCode -ne 0){throw $Process.ExitCode}
            ...
            if(Get-Job -State "Completed"){
                ...
                if($($ReturnCode) -notin $($UpgradeInfo.IgnoredExitCodes)){
                    ...
                    $WPFWindow.tbx_installationdone.text = "Installation failed with errorcode: $($ReturnCode)"
                }
                $WPFWindow.grd_installationdone.Visibility = "Visible"
                $WPFWindow.grd_batteryinfo.Visibility = "Hidden"
            }
...
Function Show-WPFMessage{
    ...
    #Load assemblys
    Start-LoadAssemblys -RootPath $FilesRoot
    ...
    if($RunningTaskSequence -eq $false){
        ...
        $NotifyIcon.BalloonTipTitle = "Firmware Update"
        $NotifyIcon.BalloonTipText = "The program will run in the background"
    ...
    #Set textboxes
    $WPFWindow.tbx_header.Text = $HeaderText
    $WPFWindow.tbx_body.Text = $BodyText
    $WPFWindow.tbx_message.Text = $BottomText
